...
# Session-specific calibration covariates used in adjusted models
pain_calib_long <- Pain_calibration %>%
  mutate(subji = as.integer(stringr::str_remove(as.character(subji), "^0+"))) %>%
  transmute(
    subji,
...
Pain_data <- Pain_ratings_wide %>%
  ...
  mutate(
    subji_num = as.integer(stringr::str_remove(as.character(subji), "^0+")),
    subji    = factor(subji_num),
  ...
  left_join(pain_calib_long, by = c("subji_num" = "subji", "session")) %>%
  mutate(
    calib_temp = if_else(intensity == "Pain", pain, no_pain)
  )
...
# Reviewer-focused sensitivity model:
# Controls for each participant's session-specific calibrated temperature.
Pain_ratings_m_temp_adj <- afex::mixed(
  rating ~ intensity * session + calib_temp + (intensity + 1 | subji),
  data   = Pain_data,
  control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 1e6)),
  method = "KR"
)
save_table(broom.mixed::tidy(Pain_ratings_m_temp_adj$full_model, effects = "fixed"),
           "pain_ratings_mixed_model_temp_adjusted")

# Nested model comparison: does calibrated temperature add explanatory value?
pain_lmer_base <- lme4::lmer(
  rating ~ intensity * session + (intensity + 1 | subji),
  data = Pain_data,
  REML = FALSE,
  control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 1e6))
)
pain_lmer_temp <- lme4::lmer(
  rating ~ intensity * session + calib_temp + (intensity + 1 | subji),
  data = Pain_data,
  REML = FALSE,
  control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 1e6))
)
pain_temp_model_comp <- as.data.frame(anova(pain_lmer_base, pain_lmer_temp)) %>%
  mutate(
    model = c("base_no_temp", "plus_calib_temp"),
    comparison = "pain_rating_model"
  )
save_table(pain_temp_model_comp, "model_comparison_pain_calib_temp")
...
roi_lmer_base <- lme4::lmer(
  beta_scale ~ intensity * session * ROI + (1 | subji),
  data = ROI_data,
  REML = FALSE,
  control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 1e6))
)
roi_lmer_temp <- lme4::lmer(
  beta_scale ~ intensity * session * ROI + calib_temp + (1 | subji),
  data = ROI_data,
  REML = FALSE,
  control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 1e6))
)
roi_temp_model_comp <- as.data.frame(anova(roi_lmer_base, roi_lmer_temp)) %>%
  mutate(
    model = c("base_no_temp", "plus_calib_temp"),
    comparison = "roi_activation_model"
  )
save_table(roi_temp_model_comp, "model_comparison_roi_calib_temp")

# Follow-up: does calib_temp contribution vary by ROI and/or session?
roi_lmer_temp_roi <- lme4::lmer(
  beta_scale ~ intensity * session * ROI + calib_temp + calib_temp:ROI + (1 | subji),
  data = ROI_data,
  REML = FALSE,
  control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 1e6))
)
roi_lmer_temp_sess <- lme4::lmer(
  beta_scale ~ intensity * session * ROI + calib_temp + calib_temp:session + (1 | subji),
  data = ROI_data,
  REML = FALSE,
  control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 1e6))
)
roi_lmer_temp_roi_sess <- lme4::lmer(
  beta_scale ~ intensity * session * ROI + calib_temp + calib_temp:ROI + calib_temp:session +
    calib_temp:ROI:session + (1 | subji),
  data = ROI_data,
  REML = FALSE,
  control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 1e6))
)

roi_temp_interaction_tests <- bind_rows(
  as.data.frame(anova(roi_lmer_temp, roi_lmer_temp_roi)) %>%
    mutate(comparison = "roi_activation_calib_temp_by_ROI"),
  as.data.frame(anova(roi_lmer_temp, roi_lmer_temp_sess)) %>%
    mutate(comparison = "roi_activation_calib_temp_by_session"),
  as.data.frame(anova(roi_lmer_temp_roi, roi_lmer_temp_roi_sess)) %>%
    mutate(comparison = "roi_activation_calib_temp_by_ROI_by_session")
) %>%
  mutate(row_id = row_number())
save_table(roi_temp_interaction_tests, "model_comparison_roi_calib_temp_interactions")

# Post-hoc/simple slopes from full interaction model
roi_calib_slopes_by_roi <- emmeans::emtrends(roi_lmer_temp_roi_sess, ~ ROI, var = "calib_temp") %>%
  summary(infer = TRUE, adjust = "fdr") %>%
  as.data.frame()
save_table(roi_calib_slopes_by_roi, "posthoc_roi_calib_temp_slopes_by_roi")

roi_calib_slopes_by_session <- emmeans::emtrends(roi_lmer_temp_roi_sess, ~ session, var = "calib_temp") %>%
  summary(infer = TRUE, adjust = "fdr") %>%
  as.data.frame()
save_table(roi_calib_slopes_by_session, "posthoc_roi_calib_temp_slopes_by_session")

roi_calib_slopes_by_roi_session <- emmeans::emtrends(roi_lmer_temp_roi_sess, ~ ROI | session, var = "calib_temp") %>%
  summary(infer = TRUE, adjust = "fdr") %>%
  as.data.frame()
save_table(roi_calib_slopes_by_roi_session, "posthoc_roi_calib_temp_slopes_by_roi_session")
...
ROI_diff <- ROI_diff %>%
  ...
  left_join(
    pain_calib_long %>%
      mutate(calib_spread = pain - no_pain) %>%
      dplyr::select(subji, session, calib_spread),
    by = c("subji_num" = "subji", "session")
  )

pain_diff_model_temp_adj <- afex::mixed(
  diff_beta ~ session * ROI + calib_spread + (1 | subji),
  data = ROI_diff,
  control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 1e6)),
  method = "KR"
)
save_table(broom.mixed::tidy(pain_diff_model_temp_adj$full_model, effects = "fixed"),
           "roi_diff_mixed_model_temp_adjusted")

roi_diff_lmer_base <- lme4::lmer(
  diff_beta ~ session * ROI + (1 | subji),
  data = ROI_diff,
  REML = FALSE,
  control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 1e6))
)
roi_diff_lmer_temp <- lme4::lmer(
  diff_beta ~ session * ROI + calib_spread + (1 | subji),
  data = ROI_diff,
  REML = FALSE,
  control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 1e6))
)
roi_diff_temp_model_comp <- as.data.frame(anova(roi_diff_lmer_base, roi_diff_lmer_temp)) %>%
  mutate(
    model = c("base_no_spread", "plus_calib_spread"),
    comparison = "roi_diff_model"
  )
save_table(roi_diff_temp_model_comp, "model_comparison_roi_diff_calib_spread")

# Follow-up interactions for diff model
roi_diff_lmer_spread_roi <- lme4::lmer(
  diff_beta ~ session * ROI + calib_spread + calib_spread:ROI + (1 | subji),
  data = ROI_diff,
  REML = FALSE,
  control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 1e6))
)
roi_diff_lmer_spread_sess <- lme4::lmer(
  diff_beta ~ session * ROI + calib_spread + calib_spread:session + (1 | subji),
  data = ROI_diff,
  REML = FALSE,
  control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 1e6))
)
roi_diff_lmer_spread_roi_sess <- lme4::lmer(
  diff_beta ~ session * ROI + calib_spread + calib_spread:ROI + calib_spread:session +
    calib_spread:ROI:session + (1 | subji),
  data = ROI_diff,
  REML = FALSE,
  control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 1e6))
)

roi_diff_spread_interaction_tests <- bind_rows(
  as.data.frame(anova(roi_diff_lmer_temp, roi_diff_lmer_spread_roi)) %>%
    mutate(comparison = "roi_diff_calib_spread_by_ROI"),
  as.data.frame(anova(roi_diff_lmer_temp, roi_diff_lmer_spread_sess)) %>%
    mutate(comparison = "roi_diff_calib_spread_by_session"),
  as.data.frame(anova(roi_diff_lmer_spread_roi, roi_diff_lmer_spread_roi_sess)) %>%
    mutate(comparison = "roi_diff_calib_spread_by_ROI_by_session")
) %>%
  mutate(row_id = row_number())
save_table(roi_diff_spread_interaction_tests, "model_comparison_roi_diff_calib_spread_interactions")

roi_diff_spread_slopes_by_roi <- emmeans::emtrends(roi_diff_lmer_spread_roi_sess, ~ ROI, var = "calib_spread") %>%
  summary(infer = TRUE, adjust = "fdr") %>%
  as.data.frame()
save_table(roi_diff_spread_slopes_by_roi, "posthoc_roi_diff_calib_spread_slopes_by_roi")

roi_diff_spread_slopes_by_session <- emmeans::emtrends(roi_diff_lmer_spread_roi_sess, ~ session, var = "calib_spread") %>%
  summary(infer = TRUE, adjust = "fdr") %>%
  as.data.frame()
save_table(roi_diff_spread_slopes_by_session, "posthoc_roi_diff_calib_spread_slopes_by_session")

roi_diff_spread_slopes_by_roi_session <- emmeans::emtrends(roi_diff_lmer_spread_roi_sess, ~ ROI | session, var = "calib_spread") %>%
  summary(infer = TRUE, adjust = "fdr") %>%
  as.data.frame()
save_table(roi_diff_spread_slopes_by_roi_session, "posthoc_roi_diff_calib_spread_slopes_by_roi_session")
...
corr_plot <- ggplot(pain_roi_diff, aes(x = pain_diff_rating, y = pain_diff_activation, color = session)) +
  ...
  facet_wrap(~ ROI, scales = "fixed", nrow = 2, labeller = as_labeller(roi_labels)) +
...
NPS_long <- NPS_long %>%
  ...
  left_join(pain_calib_long, by = c("subji_num" = "subji", "session")) %>%
  mutate(
    calib_temp = if_else(intensity == "high", pain, no_pain)
  )
...
NPS_m_temp_adj <- afex::mixed(
  value ~ intensity * session + calib_temp + (intensity + session + 1 | subji),
  data = NPS_long,
  control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 1e6)),
  method = "KR"
)
save_table(broom.mixed::tidy(NPS_m_temp_adj$full_model, effects = "fixed"),
           "nps_mixed_model_temp_adjusted")

nps_lmer_base <- lme4::lmer(
  value ~ intensity * session + (intensity + session + 1 | subji),
  data = NPS_long,
  REML = FALSE,
  control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 1e6))
)
nps_lmer_temp <- lme4::lmer(
  value ~ intensity * session + calib_temp + (intensity + session + 1 | subji),
  data = NPS_long,
  REML = FALSE,
  control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 1e6))
)
nps_temp_model_comp <- as.data.frame(anova(nps_lmer_base, nps_lmer_temp)) %>%
  mutate(
    model = c("base_no_temp", "plus_calib_temp"),
    comparison = "nps_model"
  )
save_table(nps_temp_model_comp, "model_comparison_nps_calib_temp")
