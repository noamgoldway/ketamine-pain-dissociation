...
.PHONY: all main supplementary dose-equiv verify check sync-s3 sync-s5 supp-fig-s2

all: main supplementary dose-equiv sync-s3 sync-s5 supp-fig-s2
...
check: verify
	@test -f output/revision/figures/Supplementary_Figure_S1_weight_cadss_submitted.png
